...
<%
	...
	if("".equals(dh_id)) {
		out.println("<script> location='"+page_code+"_in.jsp'; </script>");
	} else {
		// 利用 JavaMail 元件寄送信件
		...
		String servmailto = dh.getString("dh_email"); 										// 設定正本收件人 e-Mail
		...
		subject = SiteSetup.getText("cp.company." + lang) + " 捐款通知信";
		...
		try {
			...
			out.println("<script> alert('信件已寄出，請至信箱收取，感謝您的支持！');location='"+page_code+"3.jsp?dh_id="+dh_id+"'; </script>");
		} catch (Exception ex) {
			System.out.println("Project:" + projectName + ", Error info:["+ex+"], File name:web/donate/donate_sendmail.jsp, Time:["+DateTimeTool.dateTimeString()+"]");
			out.println("<script> alert('已完成您訊息的登錄，但系統發生通知信件發送失敗 !!');location='"+page_code+"3.jsp?dh_id="+dh_id+"'; </script>");
		} finally {
			app_sm.close();
		}
	}
