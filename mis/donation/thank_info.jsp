...
<%
	// 基本參數
	String code 		= "thank_info"; 					// 模組識別碼
	String show_title 	= "感謝狀內容設定";				// 模組標題
	...
	int add_num = 4;					// 設定可新增的資料筆數 , -1 為無限筆
...
/*--------------------------------------------------------------------------------------------*/
	...
	for(int i = 0; i < titles.length; i++) {
		TableRecord cp = app_sm.select(tblcp, "cp_code = ? and cp_category=? AND cp_lang=? ", new Object[]{ code, categorys[i], lang });
	 	
		if(cp.getString("cp_id").equals("")) {
	     	cp = new TableRecord(tblcp);
	     	cp.setValue("cp_title", titles[i]);
	     	cp.setValue("cp_category", categorys[i]);
	     	cp.setValue("cp_showseq", i);
	...
	cps = app_sm.selectAll(tblcp, "cp_code=? and cp_lang=?", new Object[] { code, lang }, "cp_showseq ASC , " + "cp_createdate DESC");
				...
				<tr>
					<td align="center" colspan="2"><table width="95%" border="0" cellspacing="1" cellpadding="0">
						<td class="system_bk-2bk"><table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
							...
							<%} else { %>
							<tr>
								<td align="center" colspan="3" class="information_title-1"><%=show_title%>查詢列表&nbsp;&nbsp;
							</tr>
							<%} %>
...
<form name="list<%=i + 1%>" id="list<%=i + 1%>" method="post">
							...
							<%} %>
							<td class="information_bk-2" colspan="3" align="center" height="26px">
