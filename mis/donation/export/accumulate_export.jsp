...
<%
	...
	session.setAttribute("accumulate_file", "start");
	...
	if(dhs == null || dhs.size() == 0) {
		...
		session.setAttribute("accumulate_file", "no");
	} else {
		try {
			// 建立新Excel
			...
			session.setAttribute("accumulate_file", "end");
		} catch (Exception e) {
			out.write("<script>alert('匯出失敗！');</script>");
			session.setAttribute("accumulate_file", "error");
