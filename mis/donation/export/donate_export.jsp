		...
		if(dhs == null || dhs.size() == 0) {
		   ...
		   session.setAttribute("donate_file", "no");
			...
			try {
				...
				// 將資料內容寫入指定檔案
				String clientFileName = app_account + "_donate_export.csv";
				...
				for(int e=0;e<field_titles.length;e++){
					if(e > 0) csv.append(",");
					csv.append(csvEscape(field_titles[e]));
				}
				csv.append("\r\n");
				...
				for(int i=0;i<dhs.size();i++) {
					...
					if(!"".equals(dh_pid) && dh_pid.contains("==")) {
						try { dh_pid = new AESDataEncryption().AESDecrypt(dh_pid); } catch(Exception e) {}
					}
					...
					for(int e=0;e<field_values.length;e++){
						if(e > 0) csv.append(",");
						csv.append(csvEscape(field_values[e]));
					}
					csv.append("\r\n");
				}
			...
			if (outputFile.exists()) {
				...
				// 輸出
				OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(outputFile), Charset.forName("MS950"));
