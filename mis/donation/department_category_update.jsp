...
<%
	...
	// 跳頁參數
	String[] names = new String[] { "npage", "_qtitle", "_qcategory", "now_level" };
	String[] values = new String[] { String.valueOf(pageno), qtitle , qcategory, String.valueOf(now_level)};
%>	
<%
try {
	// A:新增,M:修改,D:刪除,S:排序
	...
	if("D".equals(action)) {
		// 刪除防呆
		Vector<TableRecord> cps = app_sm.selectAll(tblcp, "cp_category=?", new Object[]{ dm_id });
		Vector<TableRecord> dms = app_sm.selectAll(tbldm, "dm_category=?", new Object[]{ dm_id });
		if(cps.size() > 0 || dms.size() > 0) {
			out.println("<script> alert('目前有資料使用此系所，故無法刪除!!'); history.back(); </script>");
			return;
		}

		TableRecord dm = app_sm.select(tbldm, dm_id);
		//FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+dm.getString("dm_image"));
		//FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+dm.getString("dm_mobile"));
		app_sm.delete(dm);

		//確認是否有其它資料顯示相同圖片(有其它檔案欄位再新增code)
		if (app_sm.success()) {
			if(app_sm.selectAll(tbldm, "dm_image=?", new Object[] { dm.getString("dm_image") }).size() == 0) {
				FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+dm.getString("dm_image"));
			}
			// 回列表頁
	...
	if("S".equals(action)) {
		...
		for(int j=0;j<list.length;j++){
			if(!"".equals(list[j].trim())) {
				TableRecord dm = app_sm.select(tbldm, "dm_id=?",new Object[] { list[j] });
				dm.setValue("dm_showseq", j);
	...
	if("A".equals(action)) {
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()){ //這是用來確定是否為文件屬性 
				...
				if(fieldName.equals("dm_title")){
					Vector checks = app_sm.selectAll(tbldm,"dm_title =? and dm_code=? and dm_lang=?",new Object[]{fieldvalue, code, lang});
					if(checks.size()>0){
				...
				dm.setValue(fieldName, fieldvalue.trim());//設定欄位值
				...
				//fileName_1 = dm.getString("dm_id")+"_"+fileName_1;
				...
				if (fileName != null && !"".equals(fileName)) {
					...
					*/
					...
					dm.setValue(fi.getFieldName(), fileName_1);//設定圖
				}
			}
		}
	...
	// 修改
		...
		TableRecord dm = app_sm.select(tbldm, "dm_id=?",new Object[] { dm_id });
		dm.setUpdate(app_account);
		while (i.hasNext()) {
			FileItem fi = (FileItem) i.next();
			if (fi.isFormField()){ //這是用來確定是否為文件屬性
				...
				if(fieldName.equals("dm_title")){
					Vector checks = app_sm.selectAll(tbldm,"dm_title =? and dm_id <>? and dm_code=? and dm_lang=?",new Object[]{fieldvalue, dm_id, code, lang});
					if(checks.size()>0){
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}else{
				...
				if (fileName != null && !"".equals(fileName)) {
					...
					*/
					...
					if(!"".equals(dm.getString(fi.getFieldName()))){
						//確認其它資料沒有使用相同檔案
						if(app_sm.selectAll(tbldm, fi.getFieldName()+"=?", new Object[] {dm.getString(fi.getFieldName()) }).size() == 1) {
							FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+dm.getString(fi.getFieldName()));
						}
					}
