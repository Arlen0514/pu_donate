...
<%
	...
	boolean keyword_switch = false;		// 是否開啟關鍵字設
	int add_num = -1;						// 設定可新增的資料筆數 , -1 為無限筆

/*--------------------------------------------------------------------------------------*/
	String ap_category = StringTool.validString(request.getParameter("category"));

	Vector aps = app_sm.selectAll(tblap, "ap_code=? and ap_lang=? and ap_category = ?", new Object[] { code, lang, ap_category}, "ap_showseq ASC , ap_createdate DESC");
...
<form name="frm" id="frm" method="post" enctype="multipart/form-data" action="<%=code%>_update.jsp?action=S&category=<%=ap_category %>" onsubmit="javascript:return checkformsequ(this);">
								<tr class="information_table-2-1">
									<td class="information_table-2-1" align="left">
										<select name="left_select" id="left_select" multiple="multiple" style="width: 660px;" size="15">
											...
											%>
											<option value="<%=data.getString("ap_id")%>"><%=data.getString("ap_title")%></option>
											<%} %>
										</select>
									</td>
									<td class="information_table-2-1" align="center" colspan="3">
