...
try {
	...
	String dm_subtitle 	= StringTool.validString(request.getParameter("dm_subtitle"));
	
	// 新增
	if("A".equals(action)){ 
		if("".equals(dm_title) || "".equals(dm_subtitle)) {
		...
		dm.setValue("dm_subtitle", dm_subtitle);
	...
	if("D".equals(action)) {
		...
		if("Y".equals(dm.getString("dm_lock"))) {
			out.println("<script> alert('此類別已鎖定，故無法刪除!!'); history.back(); </script>");
			return;
		}
		
		// 刪除防呆
		Vector<TableRecord> dhs = app_sm.selectAll(tbldh, "dh_currency=?", new Object[]{ dm.getString("dm_subtitle") });
		if(dhs.size() > 0) {
	...
	if("M".equals(action)) {
		...
		TableRecord dm = app_sm.select(tbldm, dm_id);
		String currency_str = dm.getString("dm_subtitle");
		boolean change_currency = !currency_str.equals(dm_subtitle);
		
		// 如果有變更幣別的英文
		if(change_currency){
			if("Y".equals(dm.getString("dm_lock"))) {
				out.println("<script> alert('此類別已鎖定，故無法修改!!'); history.back(); </script>");
				return;
			} else {
				Vector<TableRecord> dhs = app_sm.selectAll(tbldh, "dh_currency=?", new Object[]{ dm.getString("dm_subtitle") });
				if(dhs.size() > 0) {
					out.println("<script> alert('目前有資料使用此類別，故無法修改!!'); history.back(); </script>");
					return;
				}
			}
		}
