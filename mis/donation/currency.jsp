...
<%
	//基本參數
	String code = "currency"; 		// 模組識別碼
	String show_title = "捐款幣別設定";		// 模組標題
...
<%-- 20250331 modify Miles top menu.jsp  改成 RWD --%>
			...
			<tr>
				...
				<td align="left" valign="middle" class="information_bigword">
					<%=show_title%>
				</td>
			</tr>
			<tr>
				...
				<table width="95%" border="0" cellspacing="1" cellpadding="0">
					<tr>
						...
						<table width="100%" border="0" align="center" cellpadding="3"
							cellspacing="1">
							<tr align="center">
								<td colspan="<%="".equals(qcategory)?"3":"2"%>" class="information_title-1">
								<%=show_title%>&nbsp;&nbsp;
								<%if (list_switch) { %>
							...
							<tr class="information_table-2-1">
								...
								<div align="center">
									中文：
									<input name="dm_title" type="text" value="" size="25" maxlength="120"/>&nbsp;&nbsp;
									英文：
									<input name="dm_subtitle" type="text" value="" size="25" maxlength="120" placeholder="如存在相關捐款紀錄，則不可修改"/>
							...
							<form name="form<%=i + 1%>" method="post">
								<tr class="information_table-2-1">
									...
									<td align="left">
										<div align="center">
											中文：
											<input name="dm_title" type="text" value="<%=dm.getString("dm_title")%>" size="25" maxlength="120"/>&nbsp;&nbsp;
											英文：
											<input name="dm_subtitle" type="text" value="<%=dm.getString("dm_subtitle")%>" size="25" maxlength="120"/>
											<input name="dm_category" type="hidden" value="<%=qcategory%>"/>
										</div>
									</td>
									<td align="left">
										...
										<%if (delete_switch) { %>
											<%if(subs.size() == 0) { %>
											<input name="delete<%=i + 1%>" type="button" value="刪除" onClick="godelete(this.form, '<%=code%>_update.jsp?action=D&dm_id=<%=dm.getString("dm_id")%>&_qcategory=<%=qcategory%>&now_level=<%=now_level%>');">
											<%} else { %>
											<input name="delete<%=i + 1%>" type="button" value="刪除" onClick="alert('下層類別有資料，故無法刪除。');" />
											<%} %>
										<%} %>
										</div>
									</td>
