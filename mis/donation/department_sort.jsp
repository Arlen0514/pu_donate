...
/*------------------------------------------------------------------------------------*/
	...
	//所屬類別
	Vector<TableRecord> dms = app_sm.selectAll(tbldm, "dm_lang=? and dm_code=?", new Object[]{ lang, code+"_category" } , "dm_showseq ASC , dm_createdate DESC");
	Map<String, String> category_title_map = new HashMap<String, String>();
	
	for(TableRecord dm:dms) category_title_map.put(dm.getString("dm_id"), dm.getString("dm_title"));
%>
<!DOCTYPE html>
<html>
<head>
								...
								<td class="information_table-2-1" align="left"><select
									...
									<%
										for (int i = 0; i < cps.size(); i++) {
											...
											boolean has_title1 = category_title_map.containsKey(cp.getString("cp_upcategory"));
											boolean has_title2 = category_title_map.containsKey(cp.getString("cp_category"));
											
											String top_title = has_title1?category_title_map.get(cp.getString("cp_upcategory")):"";
											String up_title  = has_title2?category_title_map.get(cp.getString("cp_category")):"";
									%>
									<option value="<%=cp.getString("cp_id")%>"><%=top_title %> - <%=up_title %> --- <%=cp.getString("cp_title")%></option>
									<%
										}
									%>
								</select></td>
