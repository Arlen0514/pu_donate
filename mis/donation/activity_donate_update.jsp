	...
	if("A".equals(action)) {		
		...
		while (i.hasNext()) {
			...
			if(fi.isFormField()) {  
				...
				if(fieldName.equals("cp_goal")) {
					if(Integer.parseInt(fieldvalue)<0){
						out.println("<script> alert('目標金額請輸大於0的正整數!!'); history.back(); </script>");
						return;
			...
			if(fi.isFormField()) {
				...
				if(fieldName.equals("cp_title")) {
					...
					if(checks.size() > 0) {
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					} else if(fieldName.equals("cp_goal")) {
						if(Integer.parseInt(fieldvalue)<0){
							out.println("<script> alert('目標金額請輸大於0的正整數!!'); history.back(); </script>");
							return;
