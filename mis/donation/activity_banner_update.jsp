...
try {
	...
	if("D".equals(action)) {
		...
		String ap_category = ap.getString("ap_category");
		...
		if(app_sm.success()) {
			...
			// 回列表頁
			out.write(HtmlCoder.getForm("listpage", code + ".jsp?cp_id="+ap_category, names, values));
	...
	if("S".equals(action)) {
		...
		while(i.hasNext()) {
			...
			if(fi.isFormField()) {
				...
				if("selData".equals(fieldName)) {
					chk_ap = fieldvalue.trim();
				}
			}
		}
		...
		if(app_sm.success()) {
			// 回排序頁
			out.write(HtmlCoder.getForm("sortpage", code + "_sort.jsp?category="+ap_category, names, values));
	...
	if("A".equals(action)) {		
		...
		if(app_sm.success()) {
			// 回列表頁
			out.write(HtmlCoder.getForm("listpage", code + ".jsp?cp_id="+ap.getString("ap_category"), names, values));
	...
	// 修改
		...
		while(i.hasNext()) {
			...
			if(fi.isFormField()) {  
				...
				if((!fieldName.startsWith("imgradio")) && (!fieldName.startsWith("imgradio_mobile"))) {
					if("qtitle".equals(fieldName)) {
