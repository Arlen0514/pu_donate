...
<%
	...
	System.out.println("Project:" + projectName + ", File name:multi_smtp_sendmail.jsp, Message:test start, Email:" + email + ", Time:" + DateTimeTool.dateTimeString());
	...
	String ssluseauth 	= st.getString("st_ssluse"); 							// 設定 Smtp SSL 是否使用
	String us_email 	= st.getString("st_serviceemailname"); 					// 設定寄件人 e-Mail
	String us_name 		= st.getString("st_serviceemail"); 						// 設定寄件人 名稱
	String servmailto 	= email; 												// 設定正本收件人 e-Mail
	String servmailcc 	= "";													// 設定副本收件人 e-Mail
	String servmailbcc	= ""; 													// 設定密件副本收件人 e-Mail
	String subject 		= ""; 													// 設定信件主旨內容
	subject 			= SiteSetup.getText("cp.company." + lang) + " SMTP測試通知信";
	...
	if(url.indexOf("https://") > -1) {
		// 信件內容產生的 JSP 檔
		content = SSLreturnContent(url + "/web/mail/smtp_test_mail.jsp?lang=" + lang);
	} else {
		// 信件內容產生的 JSP 檔
		Vector urlcontent = HttpURL.returnContent(url + "/web/mail/smtp_test_mail.jsp?lang=" + lang);
	...
	boolean sessionDebug = true; 			// 設定是否啟用除錯模式 true 為開啟 , false 為關閉
	...
	try {
		...
		System.out.println("Project:" + projectName + ", File name:multi_smtp_sendmail.jsp, Message:test end, Email:" + email + ", Time:" + DateTimeTool.dateTimeString());
		out.println("<script> alert('通知信件寄送成功!!'); location='multi_smtp_c.jsp?st_id="+st_id+"'; </script>");
		return;
	} catch (Exception ex) {
		// 寄信前可以先開F12，這樣就能直接在console看到例外錯誤的訊息。
		System.out.println("Project:" + projectName + ", File name:multi_smtp_sendmail.jsp, Message:test error, Error:" + ex + ", Time:" + DateTimeTool.dateTimeString());
		out.println("<script> console.log('錯誤原因：" + ex.toString().replaceAll("(\r\n|\n)", " ") + "'); </script>");
		out.println("<script> alert('通知信件寄送失敗!!'); location='multi_smtp_c.jsp?st_id="+st_id+"'; </script>");
	} finally {
		app_sm.close();
	}
%>