...
try {
	...
	if("D".equals(action)) {
		...
		//FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+st.getString("st_image"));
		//FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+st.getString("st_mobile"));
		...
		if (app_sm.success()) {
			/*
			if(app_sm.selectAll(tblst, "st_image=?", new Object[] { st.getString("st_image") }).size() == 0) {
				FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+st.getString("st_image"));
			}
			*/
			// 回列表頁
	...
	if("S".equals(action)) {
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()) {											// 這是用來確定是否為文件屬性 
				...
				if("selData".equals(fieldName)){chk_st = fieldvalue.trim();}
			}
		}
		String list[] = chk_st.split(",");
		for(int j=0;j<list.length;j++){
			if(!"".equals(list[j].trim())) {
				TableRecord st = app_sm.select(tblst, "st_id=?",new Object[] { list[j] });
				st.setValue("st_showseq", j);
				st.setUpdate(app_account);
				app_sm.update(st);
			}
		}
		app_sm.close();
	...
	// 新增
	if("A".equals(action)) {		
		TableRecord st = new TableRecord(tblst);
		st.setInsert(app_account);
		//---------------------檔案上傳-----------------------------------------------
		while (i.hasNext()) {
			FileItem fi = (FileItem) i.next();
			if (fi.isFormField()) { 									// 這是用來確定是否為文件屬性
				...
				String fieldvalue = new String(fi.getString("UTF-8")); 	// 取得值
				/*
				if(fieldName.equals("st_hostname")) {
					Vector checks = app_sm.selectAll(tblst,"st_hostname =? and st_code=? and st_lang=?",new Object[]{fieldvalue, code, lang});
					if(checks.size()>0){
				...
				*/
				st.setValue(fieldName, fieldvalue.trim());				// 設定欄位值
			...
			// 處理文件
				...
				//fileName_1 = st.getString("st_id")+"_"+fileName_1;
				...
				if (fileName != null && !"".equals(fileName)) {
					...
					*/
					if((fSize*1024) < fi.getSize()) {
						out.println("<script> alert('上傳檔案不可超過"+String.valueOf(fSize)+"KB'); history.back(); </script>");
						return;
					}
					fi.write(new File(dir, fileName_1));
					st.setValue(fi.getFieldName(), fileName_1);			// 設定圖
				}
			}
		}
		st.setValue("st_code", code);	// 識別碼
		st.setValue("st_lang", lang);	// 語系		
		app_sm.insert(st);
	...
	// 修改
		...
		TableRecord st = app_sm.select(tblst, "st_id=?",new Object[] { st_id });
		st.setUpdate(app_account);
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()) {
				...
				if(!fieldName.startsWith("imgradio")) {
					...
					} else  if(fieldName.startsWith("st_")){
						st.setValue(fieldName, fieldvalue.trim());			// 設定欄位值
					}
			    }
			...
			// 處理文件(file)				
				...
				if (fileName != null && !"".equals(fileName)) {
					...
					*/
					...
					if(!"".equals(st.getString(fi.getFieldName()))) {
						// 確認其它資料沒有使用相同檔案
						if(app_sm.selectAll(tblst, fi.getFieldName()+"=?", new Object[] {st.getString(fi.getFieldName()) }).size() == 1) {
							FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+st.getString(fi.getFieldName()));
						}
					}
