...
<%
	...
	System.out.println("Project:" + projectName + ", File name:smtp_sendmail.jsp, Message:test start, Email:" + email + ", Time:" + DateTimeTool.dateTimeString());
	...
	try {
		...
		System.out.println("Project:" + projectName + ", File name:smtp_sendmail.jsp, Message:test end, Email:" + email + ", Time:" + DateTimeTool.dateTimeString());
		out.println("<script> alert('通知信件寄送成功!!'); location='smtp_c.jsp'; </script>");
		return;
	} catch (Exception ex) {
		// 寄信前可以先開F12，這樣就能直接在console看到例外錯誤的訊息。
		System.out.println("Project:" + projectName + ", File name:smtp_sendmail.jsp, Message:test error, Error:" + ex + ", Time:" + DateTimeTool.dateTimeString());
		out.println("<script> console.log('錯誤原因：" + ex.toString().replaceAll("(\r\n|\n)", " ") + "'); </script>");
		out.println("<script> alert('通知信件寄送失敗!!'); location='smtp_c.jsp'; </script>");
	} finally {
		app_sm.close();
	}
%>