				...
				if(fieldName.equals("cp_title")) {
					...
					if(fieldName.startsWith("imgradio")) {
						if (fieldvalue.trim().equals("delete")) {
							imgradio = "";
							// 確認沒有其它產品使用此圖檔
							if(app_sm.selectAll(tblcp,"cp_image=?",new Object[]{cp.getString("cp_image")}).size()==0 && app_sm.selectAll(tblcp,"pd_image=?", new Object[] {cp.getString("cp_image")}).size() == 0 ){
								FileTool.deleteFile(app_uploadpath + "/" + code + "/" + lang + "/" + cp.getString("cp_image"));
							}
							cp.setValue("cp_image", imgradio);
						}
					}					
		    	}
